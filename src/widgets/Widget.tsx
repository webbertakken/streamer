...
export function useWidgetFont(instanceId: string): string {
  const fontFamily = useOverlayStore(
    (s) => s.instances.find((i) => i.instanceId === instanceId)?.fontFamily,
  )
...
export function useWidgetBgColour(instanceId: string): string {
  const override = useOverlayStore(
    (s) => s.instances.find((i) => i.instanceId === instanceId)?.bgColour,
  )
...
export function useWidgetBgOpacity(instanceId: string): number {
  const override = useOverlayStore(
    (s) => s.instances.find((i) => i.instanceId === instanceId)?.bgOpacity,
  )
...
export function useWidgetTextColour(instanceId: string): string {
  const override = useOverlayStore(
    (s) => s.instances.find((i) => i.instanceId === instanceId)?.textColour,
  )
...
export function Widget({ instanceId, name, children }: WidgetProps) {
  ...
  const handleResizeMove = (e: React.PointerEvent) => {
    ...
    for (const g of [GRID, vw / 2, vw - GRID]) {
      if (Math.abs(rightEdge - g) < MAGNET) {
        w = g - instance.x
        break
      }
    }
    for (const g of [GRID, vh / 2, vh - GRID]) {
      if (Math.abs(bottomEdge - g) < MAGNET) {
        h = g - instance.y
        break
      }
    }
  ...
  const handleToggleSettings = useCallback((e: React.PointerEvent) => {
    e.stopPropagation()
  ...
  return (
    ...
    >
      {editMode && (
        <div
          className="absolute inset-0 border-2 border-dashed border-blue-400 pointer-events-none z-10"
          style={{ borderRadius }}
        >
          <div
          ...
          >
            <span className="text-xs text-blue-300 bg-black/60 px-1 rounded">{name}</span>
          </div>
      ...
      <div
        ...
        style={{
          ...
          backgroundColor:
            !editMode || previewBg ? hexToRgba(effectiveBgColour, effectiveBgOpacity) : undefined,
...
function WidgetSettingsPopover({
  ...
  return (
    ...
    >
      <div
        className="bg-black/80 backdrop-blur-sm p-3 space-y-3 min-w-48 border border-white/10"
        style={{ borderRadius }}
      >
                  ...
                  <button
                    ...
                    onClick={() =>
                      updateInstance(instanceId, {
                        contentAlign: instance.contentAlign === a ? undefined : a,
                      })
                    }
            ...
            {!SELF_FONT_WIDGETS.has(instance.typeId) && (
              <div className="space-y-1">
                ...
                <FontPicker
                  ...
                  onChange={(v) =>
                    updateInstance(instanceId, {
                      fontFamily: v === 'inherit' || !v ? undefined : v,
                    })
                  }
                />
              </div>
