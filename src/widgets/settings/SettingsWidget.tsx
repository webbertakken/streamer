    ...
    <div className="space-y-2">
      ...
      {authenticated ? (
        ...
        <div className="space-y-1.5">
          <p className="text-white/60 text-xs">Enter this code on the Twitch page that opened:</p>
...
function MonitorSelection() {
  ...
  async function handleToggle(id: string, checked: boolean) {
    const next = checked ? [...selectedMonitors, id] : selectedMonitors.filter((m) => m !== id)
...
function SoundMappingRow({ eventType }: { eventType: string }) {
  ...
  const mapping: SoundMapping = soundMappings[eventType] ??
    DEFAULT_SOUND_MAPPINGS[eventType] ?? { enabled: false, sound: 'chime' }
...
export function SettingsWidget() {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex ${justifyClass} ${alignClass} p-8 pointer-events-none`}
    >
