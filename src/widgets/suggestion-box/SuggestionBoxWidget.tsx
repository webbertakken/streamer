...
function SuggestionBoxContent({ instanceId }: { instanceId: string }) {
  ...
  const config: SuggestionBoxConfig = {
    ...DEFAULT_CONFIG,
    ...(instance?.config as Partial<SuggestionBoxConfig>),
  }
  ...
  useEffect(() => {
    setConfig({
      ...
      voteTrigger: config.voteTrigger,
      suggestTrigger: config.suggestTrigger,
    })
  ...
  return (
    <div ref={containerRef} className="h-full flex flex-col min-w-0">
      <div
        ref={scrollRef}
        className={`flex-1 min-h-0 min-w-0 overflow-y-auto p-2 space-y-1 scrollbar-thin flex flex-col ${alignCls}`}
      >
        {visibleActive.map((s) => (
          <SuggestionRow
            key={s.id}
      ...
      <div>
        ...
        {!config.rewardId && (
          <p className="text-xs text-amber-400/70 mt-0.5">
            No reward selected — accepting suggestions via{' '}
            <code className="bg-white/10 px-0.5 rounded">{config.suggestTrigger}</code> chat command
          </p>
        )}
      </div>
