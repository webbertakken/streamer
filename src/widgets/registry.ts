...
import {
  StreamTitleWidget,
  StreamTitleSettings,
  DEFAULT_CONFIG as STREAM_TITLE_DEFAULT_CONFIG,
} from './stream-title/StreamTitleWidget'
...
registerWidget({
  ...
  singleton: true,
  defaults: { x: 8, y: 840, width: 416, height: 296 },
  defaultConfig: { hideCommands: false },
  settingsComponent: ChatSettings,
})
registerWidget({
  id: 'viewer-count',
  name: 'Viewer count',
  ...
  singleton: true,
  defaults: { x: 1192, y: 24, width: 200, height: 64 },
})
registerWidget({
  id: 'follower-alerts',
  name: 'Follower alerts',
  component: FollowerAlertWidget,
  singleton: true,
  defaults: { x: 1112, y: 1000, width: 352, height: 120 },
})
registerWidget({
  id: 'event-feed',
  name: 'Event feed',
  ...
  singleton: true,
  defaults: { x: 2256, y: 896, width: 296, height: 272 },
})
registerWidget({
  id: 'custom-text',
  name: 'Custom text',
  component: CustomTextWidget,
  defaults: { x: 1024, y: 96, width: 544, height: 48 },
  defaultConfig: {
...
registerWidget({
  id: 'chat-presence',
  name: 'Chat presence',
  component: ChatPresenceWidget,
  singleton: true,
  defaults: { x: 2392, y: 48, width: 152, height: 568 },
})
registerWidget({
  id: 'follow-events',
  name: 'Follow events',
  component: FollowEventsWidget,
  singleton: true,
  defaults: { x: 2256, y: 648, width: 296, height: 224 },
})
registerWidget({
  id: 'event-log',
  name: 'Event log',
  ...
  singleton: true,
  defaults: { x: 8, y: 1168, width: 416, height: 216 },
})
registerWidget({
  id: 'raid-alerts',
  name: 'Raid alerts',
  component: RaidAlertWidget,
  singleton: true,
  defaults: { x: 1112, y: 880, width: 352, height: 120 },
})
registerWidget({
  id: 'subscription-alerts',
  name: 'Subscription alerts',
  component: SubAlertWidget,
  singleton: true,
  defaults: { x: 1112, y: 760, width: 352, height: 120 },
})
registerWidget({
  id: 'stream-info',
  name: 'Stream info',
  component: StreamInfoWidget,
  defaults: { x: 1192, y: 96, width: 304, height: 152 },
  defaultConfig: { showTitle: true, showGame: true, showUptime: true, showViewers: false },
  settingsComponent: StreamInfoSettings,
})
registerWidget({
  id: 'stream-title',
  name: 'Stream title',
  component: StreamTitleWidget,
  singleton: true,
  defaults: { x: 8, y: 8, width: 400, height: 48 },
  defaultConfig: { ...STREAM_TITLE_DEFAULT_CONFIG },
  settingsComponent: StreamTitleSettings,
})
registerWidget({
  id: 'suggestion-box',
  name: 'Suggestion box',
