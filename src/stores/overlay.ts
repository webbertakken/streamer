    ...
    const config =
      typeId === 'custom-text'
        ...
        : def.defaultConfig
          ...
          : undefined
    return [
      { instanceId: `${typeId}-1`, typeId, ...def.defaults, visible: true, locked: false, config },
    ]
  })
}
  ...
  return create<OverlayStore>((set, get) => ({
    ...
    addInstance: (typeId) => {
      ...
      set((s) => ({
        instances: [
          ...s.instances,
          { instanceId, typeId, ...def.defaults, visible: true, locked: false, config },
        ],
      }))
    },
    ...
    updateInstance: (instanceId, partial) =>
      set((s) => ({
        instances: s.instances.map((i) => (i.instanceId === instanceId ? { ...i, ...partial } : i)),
      })),
    fileLogging: true,
...
export const useOverlayStore: ReturnType<typeof createOverlayStore> =
  (import.meta.hot?.data?.overlayStore as ReturnType<typeof createOverlayStore>) ??
  createOverlayStore()
